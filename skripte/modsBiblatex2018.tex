...
\renewbibmacro*{url+urldate}{%
  ...
  \ifentrytype{online}
    {\setunit*{\addspace}%
     \iffieldundef{year}
       {}% kein Datum -> nichts ausgeben (Label zeigt bereits 'o. J.')
       {\usebibmacro{date}}%
       \setunit*{\addspace}%
       \usebibmacro{urldate}}%
...
% Nur \field{year} (aus echtem date), nicht \field{labelyear}: sonst wuerde
% labelyear bei datumslosen Eintraegen auf urldate zurueckfallen und diese
% faelschlich mit echt datierten Quellen desselben Autors kollidieren lassen.
\DeclareExtradate{
  \scope{
    \field{year}
    }
...
%% Online-Quellen geben das Jahr wie alle anderen Typen aus (kein eigener
%% Komma-Delimiter), damit \textcite die Jahreszahl in Klammern setzt.
...
% Einheitlicher Abstand zwischen allen Eintraegen (auch bei Wechsel des
% Anfangsbuchstabens): \bibitemsep und \bibinitsep auf denselben Wert.
\setlength{\bibitemsep}{0.5cm}
\setlength{\bibinitsep}{0.5cm}
...
\renewbibmacro*{cite}{%
  \iffieldundef{shorthand}
    ...
    {\printtext[bibhyperref]{\ifthenelse{\ifnameundef{labelname}\OR\iffieldundef{labelyear}}
      ...
      {\printnames[familyonly]{labelname}%
...
% ----------------------------------------------------------------------
% Narrative Zitate im Fliesstext mit \textcite ergeben "Nachname (Jahr)",
% z. B. "Celik & Mahmoud (2025) systematisieren ...". Es werden nur die
% Nachnamen ausgegeben und mit & verbunden. Parenthetische Zitate mit
% \cite bleiben unveraendert (z. B. "(Hou, X. et al., 2024)").
% ----------------------------------------------------------------------
...
% Wie familyonly, aber als klickbarer Verweis auf das Literaturverzeichnis.
% Wird fuer \textcite genutzt, da dessen Ausgabe nicht ohnehin schon in
% \printtext[bibhyperref]{...} gewrappt ist (anders als bei \cite).
\DeclareNameFormat{familyonlyhyper}{%
...
\let\fomorigtextcite\textcite
\renewrobustcmd*{\textcite}{%
  \AtNextCite{%
    \DeclareNameAlias{labelname}{familyonlyhyper}%
